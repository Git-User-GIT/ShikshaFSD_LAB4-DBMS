Query 1) You are required to create tables for supplier,customer,category,product,supplier_pricing,order,rating to store the data for the E-commerce with the schema definition given below. 
Create database EcommerseOrder;
use EcommerseOrder;
 
CREATE TABLE EcommerseOrder.supplier(
...
CREATE TABLE EcommerseOrder.customer(
...
CREATE TABLE EcommerseOrder.category (
CAT_ID INT NOT NULL,
CAT_NAME VARCHAR(20) NOT NULL,
PRIMARY KEY (CAT_ID)
);

CREATE TABLE EcommerseOrder.product (
PRO_ID INT NOT NULL,
...
Query 2) You are required to develop SQL based programs (Queries) to facilitate the Admin team of the E-Commerce company to retrieve the data in summarized format - The Data Retrieval needs are described below
...
SELECT 
    product.pro_name as product_name, `order`.*
...
SELECT * FROM  supplier WHERE supplier.supp_id IN (SELECT supp_id  FROM supplier_pricing
 GROUP BY supp_id  HAVING COUNT(supp_id) > 1) GROUP BY supplier.supp_id;
...
FROM  category
 INNER JOIN
(SELECT product.cat_id, product.pro_name AS p, t2.*
    FROM product INNER JOIN (SELECT pro_id, MIN(supp_price) AS Min_Price
    FROM supplier_pricing GROUP BY pro_id) AS t2 WHERE t2.pro_id = product.pro_id) AS t3
...
Query 10) Create a stored procedure to display supplier id, name, Rating(Average rating of all the products sold by every customer) and Type_of_Service. For Type_of_Service, If rating =5, print “Excellent Service”,If rating >4 print “Good Service”, If rating >2 print “Average Service” else print “Poor Service”. Note that there should be one rating per supplier.
